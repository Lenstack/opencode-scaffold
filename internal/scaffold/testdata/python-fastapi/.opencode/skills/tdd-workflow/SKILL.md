---
name: tdd-workflow
description: TDD protocol for Python + FastAPI — write failing tests first, execute after implementation, report JSON
license: MIT
...
```bash
pytest -v --asyncio-mode=auto 2>&1 | tee /tmp/test-output.txt
echo "Exit: $?"
```
