...
import (
	...
	"github.com/Lenstack/opencode-scaffold/internal/config"
...
func makeEmptyPlan(opts Options, ctx tmpl.Context) Plan {
	...
	cfg := config.New(ctx.Model, ctx.SmallModel)
	cfgContent, err := cfg.Render()
	if err == nil {
		plan.Files = append(plan.Files, FileOp{
			Path:    "opencode.json",
...
func buildConfig(ctx tmpl.Context, agents []string) *config.Config {
	cfg := config.New(ctx.Model, ctx.SmallModel)
