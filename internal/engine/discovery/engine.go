...
func New(root string, db *hub.Engine) *Engine {
...
func (e *Engine) Run(full bool) (*hub.ProjectMap, error) {
	start := time.Now()

	if !full {
		var pm hub.ProjectMap
		if err := e.db.Get(hub.NSDiscovery, "project_map", &pm); err == nil && pm.Checksum != "" {
	...
	if err := e.db.Put(hub.NSDiscovery, "project_map", pm); err != nil {
		return nil, err
	}

	for _, f := range files {
		hash := fmt.Sprintf("%x", sha256.Sum256([]byte(f.Path)))[:12]
		e.db.Put(hub.NSDiscovery, "file:"+hash, f)
	}

	for _, route := range pm.APIRoutes {
		e.db.Put(hub.NSDiscovery, "api_route:"+route.Endpoint, route)
	}

	for _, table := range pm.DBTables {
		e.db.Put(hub.NSDiscovery, "db_table:"+table.Name, table)
	}
...
func (e *Engine) scanFiles() ([]hub.FileEntry, error) {
	var files []hub.FileEntry
	var mu sync.Mutex
...
func (e *Engine) extractDBTables() []hub.DBTable {
	var tables []hub.DBTable
	...
	for _, m := range matches {
		...
		for _, line := range strings.Split(content, "\n") {
			if strings.HasPrefix(strings.TrimSpace(line), "CREATE TABLE") {
				...
				if len(parts) >= 3 {
					tables = append(tables, hub.DBTable{Name: strings.TrimSuffix(parts[2], "(")})
				}
			}
		}
	}
...
func (e *Engine) extractAPIRoutes() []hub.APIRoute {
	var routes []hub.APIRoute
	matches, _ := filepath.Glob(filepath.Join(e.root, "encore/**/*.go"))
	for _, m := range matches {
		content := readFile(e.root, m)
		for _, line := range strings.Split(content, "\n") {
			if strings.Contains(line, "//encore:api") {
				route := hub.APIRoute{Handler: m}
